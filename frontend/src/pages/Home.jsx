...
const Home = () => {
    ...
    return (
        <div className={`w-full p-5 pb-0 ${theme === "light" ? "bg-ii" : "bg-main"} flex flex-col justify-between`}>
            <div className='flex flex-col'>
                <header className='flex items-center justify-between'>
                    <h1 className={`text-lg sm:text-xl md:text-2xl font-play ${theme === "light" ? "text-main" : "text-vv"}`}>KeyKeep</h1>
                    <div className='flex items-center gap-4'>
                        <button className={`text-xl font-bold ${theme === "light" ? "text-main" : " text-vv"} cursor-pointer outline-none`} onClick={toggleTheme}>
                            {
                                theme === "dark" ? <i class="ri-sun-fill"></i> : <i class="ri-moon-fill"></i>
                            }
                        </button>
                ...
                <div className={`text-center pb-4 ${theme === "dark" ? "text-vv" : "text-main"} text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold`}>
                    A password manager that{" "}
                    <span className="relative inline-block mx-1">
                        <span className={`relative z-10 ${theme === "dark" ? "text-vv" : "text-main"}`}>Sync</span>
                        <span className={`absolute -left-2 sm:top-1 w-[150%] sm:w-[140%] h-[90%] border-2 ${theme === "dark" ? "border-v" : "border-i"} rounded-[50%] -rotate-12 z-0`}></span>
                    </span>{" "}
                    across devices.
                </div>
            </div>
            <footer className={`${theme === "dark" ? "text-vv" : "text-main"} text-xs sm:text-base font-libre text-center py-1`}>
